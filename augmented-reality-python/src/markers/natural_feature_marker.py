...
# ------------------------
#   Natual Feature Marker
# ------------------------
class NaturalFeatureMarker(MarkerBase):
    def __init__(self):
        super(NaturalFeatureMarker, self).__init__()
    ...
    def run_bf_matcher(self):
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            num, self.__r_vec, self.__t_vec, Ns = cv2.decomposeHomographyMat(H, self.__cam_mat)
            if self.__json_params["debug_draw"]:
                ...
                self.__in_image = cv2.polylines(self.__in_image, [np.int32(dst)], True, 255, 3, cv2.LINE_AA)

    def run_flann_matcher(self):
        # set FLANN matcher with default params
        ...
        if len(good) > MIN_MATCH_COUNT:
            ...
            if self.__json_params["debug_draw"]:
                # draw axis
                aruco.drawAxis(self.__in_image, self.__cam_mat, self.__dist_mat, self.__r_vec[1], self.__t_vec[1], 0.1)
                self.__in_image = cv2.polylines(self.__in_image, [np.int32(dst)], True, 255, 3, cv2.LINE_AA)
